...
export default function CreateListing() {
  ...
  const deliveryFee = useMemo(() => {
    const base = 2.5; // Plus bas pour le "last-mile"
    const distRate = 0.9; // Récompensé au KM
    const itemRate = 0.15; // Volume
    const complexity = isColis ? 3.0 : 0;

    // Minimum delivery fee of 4€
    return Math.max(
      4.0,
      parseFloat(
        (base + dist * distRate + items.length * itemRate + complexity).toFixed(
          2,
        ),
      ),
    );
  ...
  const trendingItems = useMemo(() => {
    const pool = [
      "Doliprane 1000mg tab",
      "Lait Demi-ecreme",
      "Oeufs x12",
      "Bananes 1kg",
      "Jambon blanc x4 (Lidl)",
      "Papier toilette Lotus x12",
      "Baguette",
      "Eau Cristalline 6x1.5L",
      "Beurre Lidl",
      "Pates Penne 500g",
      "Coca-Cola 1.5L",
      "Chips Lidl",
  ...
  const estimatePrice = (name: string) => {
    ...
    const match = Object.keys(PRICE_DB).find(
      (k) => key.includes(k) || k.startsWith(key),
    );
      ...
      async (pos) => {
        ...
        try {
          ...
          if (!response || !response.ok) {
            ...
            if (target === "pickup")
              setPickupLocation("Adresse trouvée par GPS");
      ...
      (err) => {
        if (retry && err.code !== err.PERMISSION_DENIED) {
          ...
          if (err.code === err.PERMISSION_DENIED) {
            alert(
              "Veuillez autoriser l'accès à votre position dans les réglages de votre iPhone.",
            );
          }
  ...
  const handlePost = async () => {
    ...
    if (
      ...
      alert(
        "Erreur Fatale Vercel: Les clés Firebase (NEXT_PUBLIC_FIREBASE_...) sont absentes ! Le site ne peut donc pas se connecter à la base de données. Allez dans les réglages de Vercel et ajoutez vos variables d'environnement.",
      );
      setSubmitting(false);
      return;
    }
    ...
    try {
      ...
      const serviceFee = isColis ? 0 : itemsTotal * 0.1;
      const deliveryFeeNum = parseFloat(String(deliveryFee)) || 0;
      const tipNum = parseFloat(String(tip)) || 0;
      const totalAmt = parseFloat(
        (itemsTotal + serviceFee + deliveryFeeNum + tipNum).toFixed(2),
      );
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={() => (stage > 1 ? setStage(stage - 1) : router.back())}
          style={{
            background: "white",
            border: "none",
            borderRadius: "50%",
        ...
        <div
          ...
          style={{
            ...
            border:
              "1px solid " +
              (location === "Localisation en cours..." ? "#007AFF" : "#f2f2f7"),
            cursor: "pointer",
            maxWidth: "180px",
            animation:
        ...
        {stage === 1 && (
          <>
            ...
            >
              ...
              <button
                onClick={() =>
                  setItems([{ name: "Colis Personnalisé", price: 0 }])
                }
                style={{
                  flex: 1,
                  padding: "8px",
            ...
            {!isColis ? (
              <>
                  ...
                  {showSuggestions && inputVal.length > 0 && (
                    ...
                    >
                      ...
                      {suggestions.map((s) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              {isPharma ? (
                                <HeartPulse size={16} color="#ff3b30" />
                              ) : (
                      ...
                      {suggestions.length === 0 && (
                        ...
                        >
                          Aucun article trouvé dans la bibliothèque.
                          <br />
                          Appuyez sur 'Entrée' pour l'ajouter manuellement.
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div
                  style={{
                    ...
                    flexWrap: "wrap",
                    gap: "10px",
                    marginBottom: inputVal.length === 0 ? "32px" : "10px",
                  }}
                >
                  {items.map((it, i) => (
                    <div
                    ...
                    >
                      ...
                      <span style={{ color: "#86868b", fontSize: "18px" }}>
                        ×
                      </span>
                    </div>
                  ))}
                    ...
                    >
                      {trendingItems.map((item) => (
                        ...
                        >
                          ...
                          >
                            {PRICE_DB[item.toLowerCase()]?.toFixed(2) || "3.00"}
                            €
                          </span>
                        </button>
                      ))}
              ...
              >
                ...
                >
                  Faites livrer n'importe quel objet <br /> (clés, sac,
                  chargeur...) à Strasbourg.
                </p>
              </div>
            )}
          </>
                        ...
                        <div
                          ...
                          onClick={() => {
                            ...
                            const road =
                              ...
                              "";
                            const suburb =
                              a.suburb || a.city_district || a.city || "";
                            const finalString =
                              `${house}${road}, ${suburb}`.trim();
            ...
            <div>
              ...
              <div style={{ position: "relative", zIndex: 5 }}>
                <input
                  ...
                  placeholder={
                    isColis ? "Adresse d'arrivée..." : "Saisir votre adresse..."
                  }
                  style={{
                    width: "100%",
                    padding: "20px",
                      ...
                      <div
                        ...
                        onClick={() => {
                          ...
                          const house = a.house_number
                            ...
                            : "";
                          const road =
                            a.road || a.pedestrian || a.suburb || s.name || "";
                          const suburb =
                            a.suburb || a.city_district || a.city || "";
                          const finalString =
                            `${house}${road}, ${suburb}`.trim();
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {dist === 0 ? "25 - 40" : Math.ceil(15 + dist * 4)} -{" "}
                      {dist === 0 ? "40" : Math.ceil(30 + dist * 4)} min
                    </div>
                  </div>
                </div>
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    border: "1.5px solid",
                    borderColor:
                      paymentMethod === "card" ? "#007AFF" : "#f2f2f7",
                    background:
                      paymentMethod === "card" ? "#f5faff" : "#fafafa",
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "1.5px solid",
                    borderColor:
                      paymentMethod === "cash" ? "#34c759" : "#f2f2f7",
                    background:
                      paymentMethod === "cash" ? "#f2fcf5" : "#fafafa",
          ...
          <div style={{ maxWidth: "600px", margin: "0 auto" }}>
            <button
              disabled={
                ...
                (stage === 2 && (!location || (isColis && !pickupLocation)))
              }
              onClick={() => {
                if (stage === 1) {
